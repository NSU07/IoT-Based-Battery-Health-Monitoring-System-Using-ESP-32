...
class MQTT2Excel:
    ...
    def on_message(self,client, userdata, msg):
        print(time.strftime("%d-%m-%Y %H:%M:%S", time.localtime())+" "+msg.topic+" "+str(msg.payload.decode("utf-8")))
        self.ws.write(self.file, 0, time.strftime("%d-%m-%Y %H:%M:%S", time.localtime()))
        
        topic = str(msg.topic)
        idx_topic = self.topics.index(topic)
        self.ws.write(self.file, (idx_topic+1), str(msg.payload.decode("utf-8")))
